...
const fetchBonusTypes = async () => {
  const query = `*[_type == "bonusType" && bookmaker->country == "Nigeria"] | order(published desc) {
    _id,
    title,
    slug,
    bookmaker->{
        ...
        <div className="flex flex-col gap-4 mb-6">
          ...
          {!loading && !error && sortedOffers.map((bonusType, idx) => (
            <div
              ...
              onClick={() => router.push(`/ng/offers/${bonusType.slug?.current}`)}
            >
              {/* Top row */}
