...
function OfferDetailsInner({ slug }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const mainBonusTypeQuery = `*[_type == "bonusType" && bookmaker->country == "Ghana" && slug.current == $slug][0]{
          _id,
          title,
          slug,
          bookmaker->{
        ...
        const totalCountQuery = `count(*[_type == "bonusType" && bookmaker->country == "Ghana" && slug.current != $slug])`;
        const total = await client.fetch(totalCountQuery, { slug });
        setTotalOffers(total);
        
        // Fetch more bonus types, excluding the current one
        const moreBonusTypesQuery = `*[_type == "bonusType" && bookmaker->country == "Ghana" && slug.current != $slug] | order(published desc)[0...$count]{
          _id,
          title,
          slug,
          bookmaker->{
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      const moreBonusTypesQuery = `*[_type == "bonusType" && bookmaker->country == "Ghana" && slug.current != $slug] | order(published desc)[$count...$newCount]{
        _id,
        title,
        slug,
        bookmaker->{
    ...
    <div className="min-h-screen bg-[#fafbfc] flex flex-col">
      ...
      <main className="max-w-7xl mx-auto w-full px-2 sm:px-4 flex-1">
        ...
        <div className="mt-6 mb-4 flex items-center gap-2 text-sm text-gray-500 ml-2">
          ...
          <Link 
            href={`/gh/${offer?.title?.toLowerCase().replace(/\s+/g, '-') || 'offers'}`} 
