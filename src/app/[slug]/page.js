...
//               <Image src="/assets/back-arrow.png" alt="Back" width={24} height={24} priority={true}/>
//               Back
//             </a>
//           </div>
...
//                           <img src={a.mainImage} alt={a.title} priority={true} className="object-cover w-full h-full" />
//                         ) : (
...
// 1. PortableText Components (Kept mostly same, but optimized Image handling)
const portableTextComponents = {
  block: {
    h1: ({ children }) => <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4">{children}</h1>,
    h2: ({ children }) => <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 mb-3">{children}</h2>,
    h3: ({ children }) => <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">{children}</h3>,
    h4: ({ children }) => <h4 className="text-base sm:text-lg font-semibold text-gray-900 mb-2">{children}</h4>,
    normal: ({ children }) => <p className="mb-4 text-gray-800 leading-relaxed">{children}</p>,
    blockquote: ({ children }) => <blockquote className="border-l-4 border-blue-500 pl-4 italic text-gray-700 mb-4">{children}</blockquote>,
  },
  list: {
  ...
  types: {
    image: ({ value }) => {
      if (!value?.asset) return null;
      // Using standard img here is okay for portable text dynamic content, 
      // but ensuring width/height are defined helps CLS
      return (
        <div className="my-4">
          <img
...
// 2. Optimized Metadata Generation (Single Request)
export async function generateMetadata({ params }) {
  const awaitedParams = await params;
  
  // Fetch document regardless of type in one go
  const doc = await client.fetch(`
    *[_type in ["hamburgerMenu", "countryPage"] && slug.current == $slug][0]{
      _type,
      title,
      country,
      metaTitle,
  ...
  // Handle Hamburger Menu Metadata
  if (doc?._type === 'hamburgerMenu') {
    return {
      title: doc.metaTitle || doc.title || 'Menu',
      description: doc.metaDescription || undefined,
      robots: {
        index: !doc.noindex,
        follow: !doc.nofollow,
      },
      alternates: { canonical: doc.canonicalUrl || undefined },
    };
  }
  
  // Handle Country Page Metadata
  if (doc?._type === 'countryPage') {
    return {
      title: doc.metaTitle || `Best Betting Sites ${doc.country} | Booldo`,
      description: doc.metaDescription || `Discover the best betting sites in ${doc.country} with exclusive bonuses and offers.`,
      robots: {
        index: !doc.noindex,
        follow: !doc.nofollow,
      },
  ...
  return {
    ...
    description: landing?.defaultMetaDescription || undefined,
    robots: {
      index: !landing?.defaultNoindex,
      follow: !landing?.defaultNofollow,
    },
    alternates: { canonical: landing?.defaultCanonicalUrl || undefined },
  };
}
...
export default async function CountryPage({ params, searchParams }) {
  ...
  // 3. Unified Fetching Logic
  // Instead of fetching Menu -> Checking -> Fetching Country -> Fetching Menu Content
  // We fetch the document once and determining what it is.
  
  // We also kick off the article fetch in parallel if it turns out to be a menu page
  // But since we don't know yet, we fetch the main doc first.
  const mainDoc = await client.fetch(`
    *[_type in ["hamburgerMenu", "countryPage"] && slug.current == $slug][0]{
      _id,
      _type,
      title,
  ...
  // === RENDER HAMBURGER MENU PAGE ===
  if (mainDoc._type === "hamburgerMenu") {
    if (mainDoc.noindex === true || mainDoc.sitemapInclude === false) {
      return (
        <div className="min-h-screen flex flex-col bg-[#fafbfc]">
          <HomeNavbar />
          <main className="flex-1 max-w-6xl mx-auto py-10 px-4 w-full text-center">
            <h1 className="text-2xl font-bold text-gray-900 mb-4 font-['General_Sans']">Content Not Available</h1>
            <p className="text-gray-600 font-['General_Sans']">This menu page is hidden.</p>
          </main>
          <Footer />
    ...
    // Fetch articles only now that we know we need them
    const articles = await client.fetch(`
      *[_type == "article" && (noindex != true) && (sitemapInclude != false)]|order(_createdAt desc)[0...5]{
    ...
    return (
      <div className="min-h-screen flex flex-col bg-[#fafbfc]">
        ...
        <main className="flex-1 max-w-6xl mx-auto py-10 px-4 w-full">
          <div className="mt-0 mb-4 flex items-center gap-2 text-sm text-gray-500 flex-wrap">
            <Link href="/" className="hover:underline flex items-center gap-1 flex-shrink-0 font-['General_Sans']" aria-label="Go back">
              <Image src="/assets/back-arrow.png" alt="Back" width={24} height={24} priority />
              Back
            </Link>
          </div>
          <div className="flex flex-col md:flex-row gap-8">
            <div className="flex-1">
              <div className="bg-white rounded-lg shadow-sm p-6 text-gray-800 leading-relaxed font-['General_Sans']">
                {mainDoc.content?.length > 0 ? (
                  <PortableText value={mainDoc.content} components={portableTextComponents} />
                ) : (
                  <div className="text-center text-gray-500 py-12">No content available.</div>
                )}
              </div>
            </div>
            
            {/* Sidebar - Articles with Next/Image optimization */}
            <aside className="w-full md:w-80 flex-shrink-0">
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 sm:p-6">
                <div className="flex items-center gap-2 mb-3 sm:mb-4">
                  <span className="text-blue-600 text-xl">●</span>
                  ...
                  {articles.map((a) => (
                    ...
                    >
                      <div className="relative w-16 h-16 flex-shrink-0 rounded overflow-hidden bg-gray-100">
                        {a.mainImage ? (
                          <Image 
                            src={urlFor(a.mainImage).width(128).height(128).auto('format').url()} 
                            alt={a.title} 
                            fill
                            sizes="64px"
                      ...
                      <div className="text-sm font-semibold text-gray-900 leading-tight font-['General_Sans'] line-clamp-2">
                        {a.title}
                      </div>
                    </Link>
                  ))}
  ...
  // === RENDER COUNTRY PAGE ===
  // At this point, we know it's a country page because of the query filter
  return (
    <CountryPageShell params={awaitedParams} hasMultipleFilters={false}>
...
// Extracted Skeleton Component for cleaner code and reusability
