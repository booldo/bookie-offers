...
const isValidAssetRef = (asset) => {
  ...
  if (asset._ref) {
    ...
    return (
      ref.startsWith("image-") && ref.length > 10 && !ref.includes("undefined")
    );
  }
...
const portableTextComponents = {
  ...
  marks: {
    strong: ({ children }) => (
      <span className="font-normal text-gray-900">{children}</span>
    ),
  ...
  types: {
    image: ({ value }) => {
      ...
      const src = imageSource
        ? imageUrlBuilder(client).image(imageSource).width(1200).url()
        : "";
    ...
    code: ({ value }) => {
          ...
          <div className="my-4">
            {value.filename && (
              <div className="bg-blue-50 px-3 py-2 text-sm text-blue-700 font-medium border border-blue-200 rounded-t">
                <span className="font-semibold">Embedded HTML:</span>{" "}
                {value.filename}
              </div>
            )}
            {value.description && (
      ...
      return (
        <div className="my-4">
          ...
          <pre
            className={`bg-[#0b1020] text-[#e2e8f0] p-4 rounded-lg overflow-x-auto text-sm ${value.filename ? "rounded-t-none" : ""}`}
          >
    ...
    codeBlock: ({ value }) => {
      ...
      return (
        <div className="my-4">
          ...
          <pre
            className={`bg-[#0b1020] text-[#e2e8f0] p-4 rounded-lg overflow-x-auto text-sm ${filename ? "rounded-t-none" : ""}`}
          >
            <code className={`language-${language || "javascript"}`}>
              {code}
            </code>
                ...
                types: {
                  ...
                  code: ({ value }) => {
                    ...
                    const shouldRenderAsHTML =
                      ...
                      language === "html" ||
                      (code && code.includes("<") && code.includes(">"));
                    console.log("Should render as HTML:", shouldRenderAsHTML, {
                      type,
                      language,
                      hasHTMLTags:
                        code && code.includes("<") && code.includes(">"),
...
function OfferDetailsInner({ slug, isPreview = false, draftId = null, initialOffer = null, initialMoreOffers = [], initialTotalOffers = 0, countrySlug = null, countryName = null }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (
          mainOffer &&
          (mainOffer.noindex === true || mainOffer.sitemapInclude === false)
        ) {
          console.log("Offer is hidden, setting hidden state");
          setIsContentHidden(true);
        }
  ...
  return (
    <>
      {/* Only show main offer content if no server-rendered content (for preview mode) */}
      {!initialOffer && (
                ...
                <div className="bg-white p-1 sm:p-6 mb-6 flex flex-col">
                  ...
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      isValidAssetRef(offer.bookmaker.logo) ? (
                        <img
      ...
      {/* More Offers Section - Always visible */}
      {moreOffers.length > 0 && (
        <div className="max-w-7xl mx-auto w-full px-0 sm:px-4 pb-24 sm:pb-0">
                  ...
                  <div className="flex flex-col gap-3 p-2 ">
                    ...
                    <div className="flex items-center justify-between min-w-0">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        isValidAssetRef(moreOffer.bookmaker.logo) ? (
                          <img
