...
import { getFilterOptionsForCountry } from "../../../lib/filterOptions";
...
export default async function CountryFiltersPage({ params }) {
  ...
  if (segments.length > 0) {
    ...
    console.log(
      "🔍 DEBUG - Affiliate link found for full path:",
      affiliateLink
    );
  ...
  const isSingleFilterPage =
    awaitedParams.filters && awaitedParams.filters.length === 1;
  ...
  if (isSingleFilterPage && singleFilter) {
    ...
    console.log(
      "🔍 DEBUG - Single filter affiliate link found:",
      affiliateLink
    );
  ...
  // Fetch filter options for this country
  const countryDoc = await client.fetch(
    `*[_type == "countryPage" && slug.current == $slug][0]{country}`,
    { slug: awaitedParams.slug }
  );
  const countryName = countryDoc?.country;
  let bonusTypeOptions = [];
  let bookmakerOptions = [];
  let advancedOptions = [];
  if (countryName) {
    const filterOptions = await getFilterOptionsForCountry(countryName);
    bonusTypeOptions = filterOptions.bonusTypeOptions;
    bookmakerOptions = filterOptions.bookmakerOptions;
    advancedOptions = filterOptions.advancedOptions;
  }
